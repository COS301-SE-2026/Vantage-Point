...
class DashBoardService:
    ...
    async def get_total_matches(session: AsyncSession) -> Any:
        try:
            statement = select(
                Matches
            )  # select all matches, don't know yet if only want a single value or not
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Could not retrieve matches. Internal server error {str(e)}",
            )

    # get db table usage, will have to wait for neo to respond before seeing fully what he want for now this should suffice
    @staticmethod
    async def get_table_storage(
        session: AsyncSession, table_name: str = "mathces"
    ) -> int:
        statement: Any = (
