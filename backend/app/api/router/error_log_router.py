...
async def list_errors(_current_user: Annotated[User, Depends(require_group(20))], session: Annotated[AsyncSession, Depends(get_session)],
        ...
        reviewed: bool | None,
        limit: int = 30,
        offset: int = 10)-> list[GetErrorLogLists]:
...
async def list_error(_current_user: Annotated[User, Depends(require_group(20))], session: Annotated[AsyncSession, Depends(get_session)],
        error_id: int)-> ErrorLog:
    response: ErrorLog = await ErrorClass.get_error_log(session, error_id)
    return response

@router.get("/dashboard/toggle/{error_id}", tags=["dashboard"])
async def toggle_reviewed(_current_user: Annotated[User, Depends(require_group(20))], session: Annotated[AsyncSession, Depends(get_session)],
        error_id: int)-> ToggleReview:
    response: ToggleReview = await ErrorClass.toggle_reviewd(error_id, session, _current_user.sub)
    return response