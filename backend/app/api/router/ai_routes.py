...
from app.pred_engine.ai_caller import (
    get_knn_output,
    get_champ_pred,
    get_item_pred,
    get_role_pred,
...
async def get_knn_output_model(
...
async def get_champ_pred_model(
    _: Annotated[User, Depends(require_group(10))],
    match_id: str,
    puuid: str,
    session: Annotated[AsyncSession, Depends(get_session)],
...
async def get_item_pred_model(
    ...
    data: Any = await live_analytics.item_data(
        match_id=match_id, puuid=puuid, session=session
    )
...
async def get_skill_pred_model(
    _: Annotated[User, Depends(require_group(10))],
    match_id: str,
    puuid: str,
    session: Annotated[AsyncSession, Depends(get_session)],
