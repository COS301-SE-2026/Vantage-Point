...
function Logo() {
  return (
    <div
      ...
      data-name="logo"
    >
      <div className="relative h-[clamp(72px,12vw,140px)] w-[clamp(72px,12vw,140px)] shrink-0">
        <img
          alt=""
...
function LandingHero({ currentSlide }: CurrentSlideProps) {
  return (
...
function SlideDots({ currentSlide, onDotClick }: FrameSlideProps) {
  return (
    <div
    ...
    >
      {SLIDE_DOT_INDICES.map((index) => (
        <button
          type="button"
          key={`landing-slide-dot-${String(index)}`}
...
export default function Landing() {
  ...
  return (
    <div className="relative w-full h-full">
      <LandingHero currentSlide={currentSlide} />
      <div className="absolute z-20 content-stretch flex flex-col items-start right-[2%] p-[16px] top-[1%] w-[clamp(180px,15vw,220px)]">
        <AuthActions />
      </div>
