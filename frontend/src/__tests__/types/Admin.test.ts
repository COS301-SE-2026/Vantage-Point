...
describe("deriveUserStatus", () => {
  it("returns 'Disabled' when the user is not enabled, regardless of user_status", () => {
    expect(
      deriveUserStatus(used({ enabled: false, user_status: "CONFIRMED" })),
    ).toBe("Disabled");
    expect(
      deriveUserStatus(
        used({ enabled: false, user_status: "FORCE_CHANGE_PASSWORD" }),
      ),
    ).toBe("Disabled");
  });
  ...
  it("returns 'Active' when enabled and user_status is CONFIRMED", () => {
    expect(
      deriveUserStatus(used({ enabled: true, user_status: "CONFIRMED" })),
    ).toBe("Active");
  });
  ...
  it.each([
    ...
    (status) => {
      expect(
        deriveUserStatus(used({ enabled: true, user_status: status })),
      ).toBe("Pending");
    },
  );
});
